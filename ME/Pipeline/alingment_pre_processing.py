...
def main(sam):
				...
				if len(q_block_ends)==2: #deveria tener solo 2 q_blocks ya que deberia encontrar una insercion asocciada a un micro-exon
					...
					if micro_exon_seq_up!="" and micro_exon_seq_down!="":  #Esto evita insersiones terminales
						...
						if (I_pos_tag - DRU <= anchor_up <= I_pos_tag + DRD):
							...
							info = (read, flag, tag, start, cigar, seq, q_block_starts, q_block_ends,  micro_exon_seq_found, I_pos_tag, DRU, DRD, DR_corrected_micro_exon_seq_found)
	...
	for i in reads_micro_exon:
		...
		read, flag, tag, start, cigar, seq, q_block_starts, q_block_ends,  micro_exon_seq_found, I_pos_tag, DRU, DRD, DR_corrected_micro_exon_seq_found = i
		intron_tag, transcript_ID, anchors = tag.split("|")
		...
		if number_of_host_introns == 1:
			...
			print "\t".join(map(str, [read, flag, tag, start, cigar, seq, q_block_starts, q_block_ends,  micro_exon_seq_found, I_pos_tag, DRU, DRD, DR_corrected_micro_exon_seq_found]))
