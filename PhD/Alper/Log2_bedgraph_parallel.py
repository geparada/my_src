...
def genome_pseudo_counts(chrom_sizes, chrom_target):
...
def bedgraph_indexer(bedgraph, pseudo_counts, chrom_target):
...
def main(bedgraph_wt, bedgraph_mut, chrom_sizes, chrom_target):

	pseudo_counts_wt = genome_pseudo_counts(chrom_sizes, chrom_target) 
	pseudo_counts_mut = pseudo_counts_wt.copy()

	counts_wt, index_wt = bedgraph_indexer(bedgraph_wt, pseudo_counts_wt, chrom_target)
	counts_mut, index_mut = bedgraph_indexer(bedgraph_mut, pseudo_counts_mut, chrom_target)
	...
	for index, row in bedgraph_df.iterrows():
		...
		WT =  row['WT']
		log2 = row['log2']
		# out = map(str, [chrom, start, end, WT, log2])
		# out = map(str, [chrom, start, end, MUT, log2])
