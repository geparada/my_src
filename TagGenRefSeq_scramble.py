    ...
    *Output = seq ID , intron name, strand, tag sequence, tag length '''
    ...
    for row in reader1:
        ...
        for t, b, t2, b2, q in zip(tstarts, blocksizes, tstarts[1:], blocksizes[1:]  , qstarts):
		    ...
		    random_tag = shuffle_word(tag)
		    print row[13] + str(tbsum) + row[8] + str(t2), random_tag, len(tag) 
		    #print ">" + row[13] + str(tbsum) + row[8] + str(t2) + "\n" + tag                
