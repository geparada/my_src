...
#	intron_list = set([])
	...
	reader = csv.reader(open(TOTAL_final_table), delimiter = ' ')
	...
	for row in reader:
		...
		if 0==0:           #Se desactiva el filtro de los no-canonicos
	...
	#							tag_complement_up = Transcriptome[qname][:qstart_up]
						...
						TOTAL_block_up = str(len(str(tag_complement_up) + str(tag_core_up)))
						TOTAL_block_down = str(len(str(tag_core_down) + str(tag_complement_down)))
