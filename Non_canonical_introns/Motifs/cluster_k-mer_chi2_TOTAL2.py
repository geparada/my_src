...
def main (non_can_fasta, can_fasta, kmer_clusters, cluster_count):
	...
	for row in csv.reader(open(cluster_count), delimiter = ' '):
		cluster_id = row[0]
		non_can_count = float(row[1])
		can_count = float(row[2])
		
		if (non_can_count >= 40 or can_count >= 40):
			high_cluster.append(cluster_id)
