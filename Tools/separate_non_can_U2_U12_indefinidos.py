...
def main(Final_table):   #La completa, con canonicos y no canonicos
	...
	for row in csv.reader(open(Final_table), delimiter = ' '):
		...
		gene, intron, chr, strand, istart, iend = row[:6]

		splice_site_5 = chr + strand + istart
		splice_site_3 = chr + strand + iend

		if dn=="GTAG" or dn=="ATAC" or dn=="GCAG":

			canonical_splice_sites.append(splice_site_5)
			canonical_splice_sites.append(splice_site_3)
	...
	for row in csv.reader(open(Final_table), delimiter = '\t'):
		...
		splice_site_5 = chr + strand + str(istart)
		splice_site_3 = chr + strand + str(iend)

		rescue = (splice_site_5 in canonical_splice_sites) or (splice_site_3 in canonical_splice_sites) or (mm9_EST_coverage + mm9_cDNA_coverage) >= 3
		...
		if dn!="GTAG" and dn!="ATAC" and dn!="GCAG":
			...
			rescue = (splice_site_5 in canonical_splice_sites) or (splice_site_3 in canonical_splice_sites) or (mm9_EST_coverage + mm9_cDNA_coverage) >= 
